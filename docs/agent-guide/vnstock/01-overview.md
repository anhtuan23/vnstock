...
2. [Giới Hạn Free-tier](#giới-hạn-free-tier)
...
This local guide copy is scoped to the free `vnstock` library only. Future
agents should prefer free `vnstock` examples and should not recommend paid-tier
libraries unless the user explicitly says their access level has changed.

### Rate Limit Tham Khảo
...
- **Layer 5 (Analytics)**: Chỉ số kỹ thuật và mô hình định giá chưa triển khai đầy đủ trong free `vnstock`.
- **Layer 6 (Macro)**: Không thuộc phạm vi free-tier guide copy này.
- **Layer 7 (Insights)**: Screener và rankings nâng cao chưa triển khai đầy đủ trong free `vnstock`.
